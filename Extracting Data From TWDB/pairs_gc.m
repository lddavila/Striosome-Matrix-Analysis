function [connectivities,retAll] = pairs_gc(twdbs, dbs, comparison_type, all_pairs, min_time, max_time, bin_size, window_size, window_shift_size, type)
%% Calculate GC connectivities...
num_windows = floor(((max_time-window_size) - min_time) / window_shift_size);
retAll = cell(1,num_windows+1);
...
for window_idx = 0:num_windows
    ...
    retAll{window_idx+1} = cell(1,length(dbs));
    gcs = cell(1,length(dbs));
    for db = 1:length(dbs)
        gcs{db} = zeros(1,size(all_pairs{db},1));
        retAll{window_idx+1}{db} = cell(1,size(all_pairs{db},1));
        for pair_num = 1:size(all_pairs{db},1)
            ...
            disp("Neuron 1 Firing Rates")
            disp(pls_spikes_binned)
            disp("Neuron 2 Firing Rates")
            disp(strio_spikes_binned)

            [GC,ret] = granger_causality(X,false);
            connectivity = GC(1,2);
            gcs{db}(pair_num) = connectivity;
            retAll{window_idx+1}{db}{pair_num} = ret; 
        end
