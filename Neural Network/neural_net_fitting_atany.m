...
n_mSPN_per_ensemble = 40;
n_FSI = 3;
n_PFCPL = 20;
n_other_ctx = 100;


sparsity_ctx_strio = 1; % on scale of 0 (no connection) to 1 (all connected)
sparsity_ctx_mSPN = 1; % on scale of 0 (no connection) to 1 (all connected)
sparsity_ctx_FSI = 1;

sparsity_FSI_strio = 1;
sparsity_FSI_mSPN = 1;%seems to affect IE pattern count the most
...
exc_thresholds = [0.4,0.5]; % threshold +/- baseline to count a pattern: strio,mSPN
inh_thresholds = [-0.5,-0.35]; % strio,mSPN
...
for j=1:100
    ...
    ctxs = {ctx_train_dat + sum(ev(:,1:2),2)',... %rev CB will emphasize both reward and cost
        ctx_train_dat + 5 * (ev(:,1)'), ... %EQR will emphasize reward dimension
        ctx_train_dat + 200 * (ev(:,2)'),... %TR will emphasize cost dimension
        ctx_train_dat + 700 * (sum(ev(:,1:n_SPN_ensembles),2)')}; %CB emphasize the cost and reward dimensions 
