...
%                     plot(N(1:100),y2,'LineWidth',2)
                ...
                title(strcat(string(currentTaskTypes(currentTaskType)), " Mixture Skewness:",string(theMixtureSkew), " Choc Skewness:",string(theChocSkew)," Concentration:", string(currentConcentrations(currentConcentration))))
                legend(strcat('Approach Mixture',string(mean(approachesToMixture))),strcat('Approach to Choc',string(mean(approachesToChoc))))
