...
n_strio_per_ensemble = 4; % for each dSPN and iSPN %this is striosome
n_mSPN_per_ensemble = 10; %this is the matrix theoretically 
n_FSI = 3; 
n_PFCPL = 20;
n_other_ctx = 100;
...
exc_thresholds = [1,1]; % threshold +/- baseline to count a pattern: strio,mSPN
inh_thresholds = [-1,-1]; % strio,mSPN
baseline_strio_activity = -1; % baseline to measure change in SPN from
...
other_ctx = ctxs{pfcpl_i}; %projects to striosome and matrix
PFC_PL = PFC_PLs{pfcpl_i}; %projects only to striosome 
