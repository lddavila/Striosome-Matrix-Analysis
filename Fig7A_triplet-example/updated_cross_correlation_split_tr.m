...
for currentDB = 2:2.5%length(twdbs) %cycle through the databases (AKA the Outer Loop in future comments)
    ...
    name_of_database_folder = strcat(namesOfDatabases(currentDB), " split"); %get name of current database
    ...
    for ttCounter = 1:length(uniqueTaskType) %cycle through all the taks types in the current database
        ...
        if contains(string(currentTaskType),"to")
