        ...
        for matrix_idx = 1:length(matrix_neuron_ids)
            for strio_idx = 1:length(strio_neuron_ids)
                ...
                for period_idx = 1:length(periods)
                    ...
                    f = figure;
                    num_bins = 10;
                    if isempty(matrix_probability_FRs)
                        continue;
                    end
                    ...
                    strio_bins = strio_bins(1:end-1) + strio_bin_size/2;
                     %% Plot the figure
%                     hold on;
%                     plot(matrix_bins, matrix_mean_resp_p, 'blue', 'LineWidth',1);
%                     scatter(matrix_bins, matrix_mean_resp_p, 'blue', 'filled');
%                     hold off;
%                     %                         xlim([10 22]); ylim([0 .7]);
%                     xlabel('Matrix Firing Rate (Hz)'); ylabel('Probability of Striosome Burst Given PLS Burst');
%                     title(strcat(string(period)," Matrix ID: ",string(matrix_id)," Strio ID: ", string(strio_id)));
%                     saveas(f, [fig_dir num2str(db) ' _' num2str(matrix_id) '_' num2str(strio_id)], 'jpg');
%                     close all;
                    
                    %% Run a Granger Causality Test
                    X = [matrix_bins;strio_bins];
