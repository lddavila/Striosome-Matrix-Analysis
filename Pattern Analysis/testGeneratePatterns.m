...
for ttCounter = 1:length(uniqueTaskType)
    ...
    currentConcentrations = unique(tableWithOnlyCurrentTaskType.conc);
    currentConcentrations = rmmissing(currentConcentrations);

    for concCounter = 0:length(currentConcentrations)
        if length(currentConcentrations)>1 && concCounter == 0
            continue
%         elseif strcmpi("CB",currentTaskType) && concCounter ==0
%             currentConc = NaN;
        else
            currentConc = currentConcentrations(concCounter);
        end
        
        cd("..\Pattern Analysis")
        [cb_strio_ids, cb_matrix_ids] = find_matrix_striosome_ids(twdbs,currentTaskType,[-Inf,-Inf,-Inf,-Inf,1],currentConc); %find neuron Ids
%         disp(cb_strio_ids{1})
...
%         %% The important part
        ...
        for currentDB=3:3.5%length(twdbs)
...
%         %% Create Figures
        ...
        if ~isnan(currentConc)
            paired_long_ttAndCMappedToEE(strcat("Task Type ", string(currentTaskType), " Concentration ", string(currentConc))) = lcounts(1,1);
            paired_long_ttAndCMappedToEI(strcat("Task Type ", string(currentTaskType), " Concentration ", string(currentConc))) = lcounts(2,1);
            paired_long_ttAndCMappedToIE(strcat("Task Type ", string(currentTaskType), " Concentration ", string(currentConc))) = lcounts(3,1);
            paired_long_ttAndCMappedToII(strcat("Task Type ", string(currentTaskType), " Concentration ", string(currentConc))) = lcounts(4,1);
            paired_long_ttAndCMappedToNP(strcat("Task Type ", string(currentTaskType), " Concentration ", string(currentConc))) = lcounts(5,1);

            unpaired_long_ttAndCMappedToEE(strcat("Task Type ", string(currentTaskType), " Concentration ", string(currentConc))) = lcounts(1,2);
            unpaired_long_ttAndCMappedToEI(strcat("Task Type ", string(currentTaskType), " Concentration ", string(currentConc))) = lcounts(2,2);
            unpaired_long_ttAndCMappedToIE(strcat("Task Type ", string(currentTaskType), " Concentration ", string(currentConc))) = lcounts(3,2);
            unpaired_long_ttAndCMappedToII(strcat("Task Type ", string(currentTaskType), " Concentration ", string(currentConc))) = lcounts(4,2);
            unpaired_long_ttAndCMappedToNP(strcat("Task Type ", string(currentTaskType), " Concentration ", string(currentConc))) = lcounts(5,2);
        else
        ...
        title("Stress2 Database Pattern Counts Long Trials, paired by Granger Causality")
        subtitle(strcat("Task Type ", string(currentTaskType), " Concentration ", string(currentConc)))
        legend("Paired","Unpaired")
        ylim([0,5.5])
        if ~isnan(currentConc)
            name = strcat("Stress2 Long Trial Pattern Counts Task Type ",string(currentTaskType)," Concentration ",string(currentConc),".fig");
        else
            name = strcat("Stress2 Long Trial Pattern Counts Task Type ",string(currentTaskType),".fig");
        end
        saveas(gcf,name);
        hold off
        close all
        ...
        if ~isnan(currentConc)
            paired_short_ttAndCMappedToEE(strcat("Task Type ", string(currentTaskType), " Concentration ", string(currentConc))) = scounts(1,1);
            paired_short_ttAndCMappedToEI(strcat("Task Type ", string(currentTaskType), " Concentration ", string(currentConc))) = scounts(2,1);
            paired_short_ttAndCMappedToIE(strcat("Task Type ", string(currentTaskType), " Concentration ", string(currentConc))) = scounts(3,1);
            paired_short_ttAndCMappedToII(strcat("Task Type ", string(currentTaskType), " Concentration ", string(currentConc))) = scounts(4,1);
            paired_short_ttAndCMappedToNP(strcat("Task Type ", string(currentTaskType), " Concentration ", string(currentConc))) = scounts(5,1);

            unpaired_short_ttAndCMappedToEE(strcat("Task Type ", string(currentTaskType), " Concentration ", string(currentConc))) = scounts(1,2);
            unpaired_short_ttAndCMappedToEI(strcat("Task Type ", string(currentTaskType), " Concentration ", string(currentConc))) = scounts(2,2);
            unpaired_short_ttAndCMappedToIE(strcat("Task Type ", string(currentTaskType), " Concentration ", string(currentConc))) = scounts(3,2);
            unpaired_short_ttAndCMappedToII(strcat("Task Type ", string(currentTaskType), " Concentration ", string(currentConc))) = scounts(4,2);
            unpaired_short_ttAndCMappedToNP(strcat("Task Type ", string(currentTaskType), " Concentration ", string(currentConc))) = scounts(5,2);
        else
        ...
        xticklabels(categories)
        title("Stress2 Database Pattern Counts Short Trials, paired by Granger Causality")
        subtitle(strcat("Task Type ", string(currentTaskType), " Concentration ", string(currentConc)))
        legend("Paired","Unpaired")

        ylim([0,5.5])
        if ~isnan(currentConc)
            name = strcat("Stress2 Short Trial Pattern Counts Task Type ",string(currentTaskType)," Concentration ",string(currentConc),".fig");
        else
            name = strcat("Stress2 Short Trial Pattern Counts Task Type ",string(currentTaskType),".fig");
        end
%         disp(name)
        saveas(gcf,name)
        hold off
