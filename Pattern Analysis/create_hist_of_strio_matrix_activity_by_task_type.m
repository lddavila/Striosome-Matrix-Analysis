...
%     disp(uniqueTaskType)
    ...
    for ttCounter = 1:length(uniqueTaskType)
        ...
        for currentPair=1:height(currentDatabasePairs)
            cd("..\Pattern Analysis")
            [result,pairedCountCurr,array_of_firsts,neuron_1_z_score,neuron_2_z_score]  = runGrangerCausalityOnlyToGetdistOfAvgAct(currentPair,currentDatabase,currentDatabasePairs,nlags,currentDB,dbs);
...
%         histogram(array_of_z_scores_for_matrix);
        ...
        xlabel("Z-Score")
        ylabel("Bin Count")
        subtitle("created by create_hist_of_strio_matrix_activity_by_task_type.m")
%         saveas(gcf,name);
        hold off; 
%         close all;
