...
for currentDB = 1:1.5%length(twdbs)
    ...
    for ttCounter = 1:1.5%length(uniqueTaskType)
        ...
        concentrationsForCurrentTaskType = rmmissing(unique(tableWithOnlyCurrentTaskType.conc));
        if currentDB==1 && strcmpi(currentTaskType,"EQR")
            concentrationsForCurrentTaskType = [nan];
        end
        if currentDB==2 && strcmpi(currentTaskType,"CB")
            concentrationsForCurrentTaskType = [nan];
        end
        disp(concentrationsForCurrentTaskType.')

        for concCounter =1:1.5%length(concentrationsForCurrentTaskType)
            disp(strcat("Current Concentration: ", string(concentrationsForCurrentTaskType(concCounter))))

            cd("..\Pattern Analysis")
            [cb_strio_ids, cb_matrix_ids] = find_matrix_striosome_ids(twdbs,currentTaskType,[-Inf,-Inf,-Inf,-Inf,1],concentrationsForCurrentTaskType(concCounter)); %find neuron Ids
            %         disp(cb_strio_ids{1})
            ...
            for currentPair=1:1.5%height(currentDatabasePairs)
                cd("..\Pattern Analysis")
